...
const logger = bunyan.createLogger({ name: 'simplifiedVideo' });
...
const CreateThumbnail = (filename: string) => {
	return new Promise((resolve, reject) => {
		const command = ffmpeg(`${UPLOAD_PATH}/${filename}`);
		...
		try {
			...
			logger.error('error in Thumbnail creation : ', err);
			reject(err);
		}
	});
};
...
export { VideoDB, UPLOAD_PATH, IMAGE_PATH, CreateThumbnail, ValidateVideo };
